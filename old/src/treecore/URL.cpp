...
void URL::createHeadersAndPostData (String& headers, MemoryBlock& headersAndPostData) const
{
    ...
    if (filesToUpload.size() > 0)
    {
        ...
        {
            const Upload& f = *filesToUpload[i];
...
URL URL::withParameters (const HashMap<String, String>& parametersToAdd) const
{
    URL u (*this);

    HashMap<String, String>::ConstIterator i(parametersToAdd);
    while (i.next())
        u.addParameter (i.key(), i.value());
...
URL URL::withUpload (Upload* const f) const
{
    ...
    for (int i = u.filesToUpload.size(); --i >= 0;)
        if (u.filesToUpload[i]->parameterName == f->parameterName)
...
String URL::removeEscapeChars (const String& s)
{
    ...
    for (int i = 0; i < utf8.size(); ++i)
    {
        if (utf8[i] == '%')
        {
            ...
            {
                utf8[i] = (char) ((hexDigit1 << 4) + hexDigit2);
