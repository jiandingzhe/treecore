...
public:
    ...
    {
        treecore_assert( *data != 0 ); // trying to advance past the end of the string?
        const signed char n = (signed char) *data++;

        if (n < 0)
        {
            treecore_wchar bit = 0x40;
    ...
    treecore_wchar operator [] ( int characterIndex ) const noexcept
    {
    ...
    {
        return CharPointer_UTF8( data + std::strlen( data ) );
    }

    /** Writes a unicode character to this string, and advances this pointer to point to the next position. */
    void write( const treecore_wchar charToWrite ) noexcept;
