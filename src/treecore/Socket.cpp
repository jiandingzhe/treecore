...
#    include <sys/types.h>
#    include <unistd.h>
typedef socklen_t _sock_len_t_;
                               ...
                               const int     timeOutMillisecs )
{
    if (isListener)
    {
        treecore_assert_false;    // a listener socket can't connect to another one!
        return false;
    }
...
#if !TREECORE_OS_WINDOWS  // on windows, adding this option produces behaviour different to posix
...
StreamingSocket* StreamingSocket::waitForNextConnection() const
{
    ...
    treecore_assert( isListener || !connected );

    if (connected && isListener)
    {
        struct sockaddr_storage address;
        _sock_len_t_ len    = sizeof(address);
