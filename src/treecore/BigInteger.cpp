...
BigInteger& BigInteger::operator = ( const BigInteger& other )
{
    ...
    {
        highestBit = other.getHighestBit();
        treecore_assert( other.numValues >= 4 );
...
uint32 BigInteger::getBitRangeAsInt( const int startBit, int numBits ) const noexcept
{
    if (numBits > 32)
    {
        treecore_assert_false;  // use getBitRange() if you need more than 32 bits..
        numBits = 32;
    }
...
{
    treecore_assert( n != 0 ); // (the built-in functions may not work for n = 0)
...
{
    treecore_assert( this != &remainder ); // (can't handle passing itself in to get the remainder)
...
BigInteger& BigInteger::operator |= ( const BigInteger& other )
{
    ...
    treecore_assert( isNegative() == other.isNegative() );
...
String BigInteger::toString( const int base, const int minimumNumCharacters ) const
{
    ...
    if (base == 2 || base == 8 || base == 16)
    {
        ...
        for (;; )
        {
            ...
            s = String::charToString( (treecore_wchar) (uint8) hexDigits [remainder] ) + s;
        }
    }
    else if (base == 10)
    {
    ...
    {
        treecore_assert_false; // can't do the specified base!
        return String();
    }
