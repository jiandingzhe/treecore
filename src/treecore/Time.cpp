...
static struct tm millisToLocal( const int64 millis ) noexcept
{
    ...
    if (seconds < 86400LL || seconds >= 2145916800LL)
    {
        ...
        const int64 timeZoneAdjustment = 31536000 - int(Time( 1971, 0, 1, 0, 0 ).toMilliseconds() / 1000);
...
{
    treecore_assert( year > 100 ); // year must be a 4-digit version
...
Time TREECORE_STDCALL Time::getCurrentTime() noexcept
{
...
{
    const uint32 now = _milli_seconds_since_startup_();
