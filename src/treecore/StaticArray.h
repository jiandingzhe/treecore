#ifndef TREECORE_STATIC_ARRAY_H
#define TREECORE_STATIC_ARRAY_H
...
public:
    ...
    ChildType& operator [] ( IntType i ) noexcept
    {
        ...
        treecore_assert( i < (IntType) InitSize0 );
        treecore_assert( (int) i >= 0 );
        return m_data[i];
    }
...
private:
    ...
    {
        check();
        treecore_assert( init.size() == size );
        T* k = get();
...
//    typename aligned_type< static_array_pod_imp<Temp,SizeQueue>, ALIGNOF(T) >::type m_rawArray;
    ...
    {
        treecore_assert( initList.size() == 1 );
        T* k = get();
    ...
    {
        treecore_assert( initList.size() == size );
        T* k = get();
